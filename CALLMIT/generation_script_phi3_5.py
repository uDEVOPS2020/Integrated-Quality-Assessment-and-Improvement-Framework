...
def __main__(causal_path_type, APP, RANGE_REP=range(10), MAX_RETRY=5):
    if not os.path.exists('results'):
        os.mkdir('results')

    base_path = os.path.join("results", 'phi3.5')
    ...
    model_id = 'CristianMascia/Phi-3.5-mini-instruct-gptq-4bit-c4-new'
    ...
    for rep in RANGE_REP:
        ...
        for metric in utility.METRICS:
            for service in services:
                ...
                )} | prompt | llm | JsonOutputParser(pydantic_object=utility.Triple)
...
for app in ['mubench', 'teastore', 'sockshop']:
    __main__(CAUSAL_PATHS.TOP5_PRUNED, app, RANGE_REP=range(10), MAX_RETRY=5)


    
    __main__(CAUSAL_PATHS.NONE, app, RANGE_REP=range(10), MAX_RETRY=5)
    __main__(CAUSAL_PATHS.STRONGEST, app, RANGE_REP=range(10), MAX_RETRY=5)
    __main__(CAUSAL_PATHS.STRONGEST_PRUNED, app, RANGE_REP=range(10), MAX_RETRY=5)
    __main__(CAUSAL_PATHS.TOP5, app, RANGE_REP=range(10), MAX_RETRY=5)
    __main__(CAUSAL_PATHS.TOP5_PRUNED, app, RANGE_REP=range(10), MAX_RETRY=5)