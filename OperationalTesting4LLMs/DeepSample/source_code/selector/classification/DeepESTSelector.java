...
public class DeepESTSelector {
	...
	public double[] selectAndRunTestCase(int n, ArrayList<TestFrame> testFrame, double[][] weightsMatrix, double d) {
		...
		int esito = testFrame.get(randomNum).extractAndExecuteTestCase(); // Return int directly
		String tc = testFrame.get(randomNum).getName();

		ak.activeSetUpdate(name, randomNum, (esito == 1), weightsMatrix);
		...
		while (k < n) {
			...
			if (prob <= d) {
				...
				esito = testFrame.get(current_tf).extractAndExecuteTestCase(); // Return int directly
				...
				ak.activeSetUpdate(name, current_tf, (esito == 1), weightsMatrix);
